...
g = sns.catplot(x="snr", y="duration", hue="system", data=results, sharey=False, kind="point", legend=False, legend_out=True,
                scale=0.6, errwidth=1.5, dodge=0.1, capsize=0.05)
plt.legend(title="Observability", loc="center left", bbox_to_anchor=(1.05, 0.5), frameon=False)
g.set(ylabel="Time / min", xlabel="SNR / dB")
plt.title("A", loc='left', pad=-30, fontweight="bold")
g.savefig(path_out + f"plot_duration_perSystems_{base_plot_name}_{plot_version}.png", dpi=300)
plt.close()
...
results_clean.TExy[results["TExy"] > 1000] = np.nan
results_clean.TExy[results["TExy"] < -1000] = np.nan

plot_version = 'p1'
g = sns.catplot(x="snr", y="TExy", hue="obs_type", data=results_clean, sharey=False,
...
# ----------------------------
results_clean = results.copy()
results_clean.RE[results["RE"] > 1000] = np.nan
results_clean.RE[results["RE"] < -1000] = np.nan

plot_version = 'p1'
g = sns.catplot(x="snr", y="RE", hue="obs_type", data=results_clean, sharey=False, kind="point",
                legend=False, legend_out=True, col="system", col_wrap=5,
                scale=0.8, errwidth=1.5, dodge=0.1, capsize=0.05)
plt.legend(title="Observability", loc="center left", bbox_to_anchor=(1.05, 0.5), frameon=False)
g.set(ylim=(0, 100), ylabel="Reconstruction Error", xlabel="SNR / dB") # ylim=(0, 10),
# plt.title("C", loc='left', pad=-30, fontweight="bold")
...
g = sns.catplot(x="snr", y="RE", hue="obs_type", data=results_clean, sharey=False, kind="point",
                ...
                scale=0.8, errwidth=1.5, dodge=0.1, capsize=0.05)
plt.legend(title="Observability", loc="center left", bbox_to_anchor=(1.05, 0.5), frameon=False)
g.set(ylim=(0, 1000), ylabel="Reconstruction Error", xlabel="SNR / dB") # ylim=(0, 10),
# plt.title("C", loc='left', pad=-30, fontweight="bold")
