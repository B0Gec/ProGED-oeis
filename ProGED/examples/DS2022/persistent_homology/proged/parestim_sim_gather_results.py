...
# sys_name, snr, iobs, obs, iinit, im = sys_names[9], snrs[0], 0, ['x', 'y'], 0, 0
        ...
        for iobs, obs in enumerate(obss):
            ...
            for iinit in range(n_inits):
                ...
                for im in range(len(systemBox)):
                    ...
                    if not systemBox[im].valid:
                        print("Not Valid: " + results_filename)
                        ires = [method, data_version, exp_version, benchmark, sys_name, snr, obs_name, iinit] + list(np.full(len(results_list[0]) - 7, np.nan))
                        results_list.append(ires)
                        continue

                    duration = systemBox[im].estimated['duration']  #seconds

                    # get estimated initial values for partial observability
                    init_model = np.copy(init_true)
                    if obs == ["x"]:
                        init_model[1] = systemBox[im].initials[0] # change y
                    elif obs == ["y"]:
                        init_model[0] = systemBox[im].initials[0] # change x

                    # gather modeled trajectories
                    sys_func = systemBox[im].lambdify(list=True)
                    system_model = System(sys_name="model",
                                          sys_vars=systemBox[im].sym_vars,
                                          sys_func=sys_func)
                    itr_model = system_model.simulate(init_model, sim_step=sim_step, sim_time=sim_time)
                    ...
                    modeled_params_unpacked = [j for i in systemBox[im].params for j in i]
                    ...
                    # gather all results
                    ires = [method, data_version, exp_version, benchmark, sys_name, snr, obs_name, iinit, init_true, init_model, duration, systemBox[im].estimated['fun'],
                            estimation_settings["optimizer_settings"]["max_iter"], estimation_settings["optimizer_settings"]["pop_size"], systems[sys_name].orig_expr, systemBox[im].expr, systems[sys_name].sym_params, systemBox[im].estimated['x'],
                ...
                if (iinit == 0 or iinit == 3) and plot_trajectories_bool and systemBox[im].valid:
                ...
                if (iinit == 0 or iinit == 3) and plot_optimization_curves_bool and systemBox[im].valid:
