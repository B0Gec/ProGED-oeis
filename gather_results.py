...
def extract_file(fname, verbosity=VERBOSITY):
    ...
    return we_found, is_reconst, is_check
...
def for_summary(aggregated: tuple, fname: str):
    ...
    # now -> f, m, i, o
    we_found, is_reconst, is_checked = extract_file(job_dir + fname)
...
print()
print(summary)
print(f'all files:{len(files)}, sum:{sum(summary)}, corrected sum: {sum(summary[:4]) - sum(summary[4:])}')
# print(f((1,2,3,4,), 'c'))
...
printout = f"""
    {id_oeis} - successfully found equations that are identical to the recursive equations written in OEIS (hereinafter - OEIS equation)
    {non_id} - successfully found equations that are more complex than the OEIS equation ... 'manually' checked but ..!hey this equals next to last item in this list!
    {non_manual} - successfully found equations that do not apply do not apply to test cases ... 'manually' unchecked wrong (eq. holds just for first few terms).
    {fail} - failure, no equation found. ... aka. We didn't found equations!
    {reconst_non_manual} - fail in program, specificali reconstructed oeis and manually wrong.
"""
print(printout)