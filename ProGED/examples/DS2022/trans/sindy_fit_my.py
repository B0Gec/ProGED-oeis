...
from parestim_sim_plots import plot_trajectories
...
# [duration, TExyz, eq_dx, eq_dy, eq_dz]
...
for sys_name, iobs, iinit, snr in combinations:
    ...
    # print(t[0], t[1], t[2], t[-1], x0)
    # model = ete_fit(X, x_dot=data_der, sys_name=sys_name)
    # print(type(model[0]))
    # 1/0
    ...
    entry[0] = duration
    ete_df[key] = entry
    # ete_filename = f"{path_base_out}{sys_name}"
    ete_df.to_csv(path_base_out + sys_name + "_ete.csv", index=False)
    ...
    # ires = [method, data_version, exp_version, benchmark, sys_name, snr, iobs_name, iinit, x0, x0,
    #         duration, np.nan, np.nan, np.nan, systems[sys_name].orig_expr, [str(eq) for eq in model],
    #         systems[sys_name].sym_params, np.nan,
    #         TEx, TEy, TExy, np.nan, num_true_params, np.nan]
    #
    # results_list.append(ires)
