...
class System():
    ...
    def __init__(self, sys_name, sys_vars, **kwargs):
        ...
        self.grammar_vars = kwargs.get('grammar_vars', [])
        self.gram_structure = kwargs.get('gram_structure', [])
        self.gram_params = kwargs.get('gram_params', [])
        self.data_column_names = kwargs.get('data_column_names', sys_vars)
        self.sys_func = kwargs.get('sys_func', [])
        self.expr_parts_yes = kwargs.get('expr_parts_yes', [])
        self.expr_parts_no = kwargs.get('expr_parts_no', [])
        self.num_samples = kwargs.get('num_samples', [])
        self.num_batches = kwargs.get('num_batches', [])
        self.sindy_params = kwargs.get('sindy_params', [])

        if not type(self.grammar_type) == 'list':
            self.grammar_type = list(self.grammar_type)
...
def simulate_check_sys_func(self):
    ...
    if sympy.Symbol("t") in self.sys_vars or "t" in self.sys_vars:
